...
| 7 | **Tambah `WITH CHECK` clause** di RLS UPDATE policy `appointments` agar new row state juga divalidasi. Saat ini hanya `USING` (old state). | Backend (SQL) | Migration `016_appointments_update_with_check.sql` | 🟡 Medium | ⬜ |
