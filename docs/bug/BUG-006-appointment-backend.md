...
| 7 | **Tambah `WITH CHECK` clause** di RLS UPDATE policy `appointments` agar new row state juga divalidasi. Validasi: patient_id masih milik user + status hanya 'cancelled'. | Backend (SQL) | `supabase/migrations/016_appointments_update_with_check.sql` | 🟡 Medium | ✅ Fixed (29 Jun) |
| 8 | **Improve `ErrorHandler`** — map `P0001` (RAISE EXCEPTION) via substring `e.message` (SLOT_ALREADY_BOOKED → conflict, NOT_FOUND → notFound, FORBIDDEN → forbidden, INVALID/CANCEL → validationError). | Flutter | `lib/core/network/error_handler.dart` | 🟡 Medium | ✅ Fixed (29 Jun) |
| 9 | **Tambah handling untuk `FunctionException`** di `ErrorHandler` — pattern match + mapping berdasarkan HTTP status dari `e.status`. | Flutter | `lib/core/network/error_handler.dart` | 🟡 Medium | ✅ Fixed (29 Jun) |
